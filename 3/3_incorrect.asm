; Lab1CompleteFixed.asm
; Исправленная полная программа на ассемблере для ввода a, b, c,
; выполнения вычислений и вывода результата.
; Добавлена обработка ошибок для неверного ввода.
; Компилируется и работает в DOSBox с TASM, x86-16.
...
.DATA
    ...
    incorrectMsg DB 13,10,'INCORRECT$',  ; Сообщение об ошибке
...
MAIN PROC
    ...
    call StrToNum
    jc IncorrectInput  ; Если ошибка, вывести "INCORRECT" и завершить
...
IncorrectInput:
    ...
    mov ah, 09h
    int 21h
    ; Завершение программы с кодом возврата 1
    mov ax, 1
    mov ah, 4Ch
    int 21h
MAIN ENDP
...
; Выход: AX = число; CF = 1, если ошибка
StrToNum PROC
...
StrToNumLoop:
    ...
    je StrToNumEnd
    cmp bl, '0'
    jb InvalidInput    ; Если символ меньше '0', ошибка
    cmp bl, '9'
    ja InvalidInput    ; Если символ больше '9', ошибка
...
StrToNumEnd:
    clc                ; Сброс флага CF (ошибки нет)
    ret

InvalidInput:
    stc                ; Установка флага CF (ошибка)
    ret
StrToNum ENDP
